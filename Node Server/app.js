...
io.on("connection", (socket) => {
  ...
  socketHandlers.updateUserRooms(io);

  //  scenario 1 - client sends server 'join' message using room to join
  socket.on("join", (client) => {
    socketHandlers.handleJoin(client, socket, io);
  });
  ...
  // scenario 4 - client sends a msg
  socket.on("message", (message) => {
    socketHandlers.handleMessage(io, socket, message);
  });

  socket.on("notTyping", () => {
    socketHandlers.handleNotTyping(socket);
  });
});
